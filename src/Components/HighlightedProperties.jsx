...
const HighlightedProperties = () => {
  ...
  return (
    <div className="highlighted-properties-container my-10 mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 md:grid-cols-2 gap-6 mx-9">
        {filteredProperties.map(property => (
          <Link to={`/highlights/${property.id}`} key={property.id} style={{maxWidth: "100%", textAlign:"center" , display:"flex",justifyContent:"center" }}>
            <div className="property-h-card max-w-96 bg-white shadow-lg rounded-lg overflow-hidden hover:shadow-xl transition-shadow duration-300">
              <img src={property.images[0]} alt={property.name} className="h-64 w-full object-fill" />
              <div className="p-4">
                <h3 className="text-sm font-bold mb-2">{property.name}</h3>
